  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setError(data.message || 'Invalid email or credentials.');
      }
    } catch (err) {
      console.error("Login Error:", err);
      setError('Server integration failed. Try again.');
    }
  };
  ...
  return (
    <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '100vh', backgroundColor: '#0f172a', color: '#fff', padding: '15px' }}>
      <div style={{ backgroundColor: '#1e293b', padding: '40px', borderRadius: '8px', width: '100%', maxWidth: '400px', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.3)' }}>
        ...
        {error && (
          <div style={{ backgroundColor: '#7f1d1d', border: '1px solid #f87171', padding: '10px', borderRadius: '4px', marginBottom: '15px', textAlign: 'center', fontSize: '14px' }}>
            {error}
          </div>
        )}
